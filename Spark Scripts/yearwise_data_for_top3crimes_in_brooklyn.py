...
if __name__ == "__main__":
    ...
    printdata = year_num.map(lambda x: x[7])
    print(printdata.collect())

    year_num_topCrime = year_num.filter(lambda x: (str(x[7])) == "ASSAULT 3 & RELATED OFFENSES")
    ...
    year_num_secondTopCrime = year_num.filter(lambda x: (str(x[7])) == "OFF. AGNST PUB ORD SENSBLTY &")
    ...
    year_num_thirdTopCrime = year_num.filter(lambda x: (str(x[7])) == "MISCELLANEOUS PENAL LAW")
